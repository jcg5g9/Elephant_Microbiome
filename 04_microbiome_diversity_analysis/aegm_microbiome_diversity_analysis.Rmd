...
## PHASE 1: DATA READ-IN, IDENTIFICATION OF MAJOR OTU GROUPS, AND FILTERING
In this phase of the analysis, we read in the 
...
# Generate dataset with total abundance and relative abundance across samples of each unique OTU
otu_relative_abundance <- order_data %>%
  group_by(otu_id) %>%
  summarize(mean_abundance = mean(abundance)) %>%
  mutate(relative_abundnance = mean_abundance/total_abundance)

otu_relative_abundance %>%
  arrange(desc(relative_abundnance))
