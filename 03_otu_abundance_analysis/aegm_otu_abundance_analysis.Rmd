...
After data summarization, we observed that individual OB182 had an order of magnitude lower read count (163) than the next most abundant individual (11,460). For this reason, we opted to drop OB182 from the final dataset for downstream analyses, and the OTU raw abundance data were rarefied to 11,460 reads (Continue on to Step 1d)
...
#### 1d. Omit sample with extremely high read count relative to other samples (CR71), and re-visualize read counts to identify low-read individuals; run the Rmd chunk below.

##### Plot OTU read counts per elephant sample without individual CR71: `figures/reads_filtered.pdf`
```{r}
# Load read count data
load("data/phy_raw.Rda")

# Remove individual OB182 from the data
phy_filtered <- subset_samples(phy_raw, ind_id != "OB182") 

# Save filtered otu abundance phyloseq object
save(phy_filtered, file = "data/phy_filtered.Rda")
```

### STEP 2: Rarefy OTU abundance data
In this step, we rarefy the OTU abundance dataset to the lowest number of reads captured in the full dataset for an individual. Rarefaction is a necessary step to randomly sample OTU abundance data to an even number of "samples", aka reads, to prevent sampling bias in downstream analyses. 
...
load("data/phy_filtered.Rda")

# Read in, combine, and convert QIIME output and metadata to phyloseq format
phy_rarefied <- rarefy_even_depth(phy_filtered, 
                                  rngsee = 5)

# Save rarefied OTU abundance data for downstream analyses
save(phy_rarefied, file = "data/phy_rarefied.Rda")
```

<b> Data filtering results </b>: <br>
<b><i>read depth</i> = 11,460 </b><br>

### STEP 3: Rarefy OTU abundance data
In this step, we rarefy the OTU abundance dataset to the lowest number of reads captured in the full dataset for an individual. Rarefaction is a necessary step to randomly sample OTU abundance data to an even number of "samples", aka reads, to prevent sampling bias in downstream analyses. 

#### 3a. Rarefy the OTU abundance data; run the Rmd chunk below.

##### Rarefy abundance data
```{r}
# Load filtered phyloseq object
load("data/phy_rarefied.Rda")

# Generate individual data table for taxonomic classifications
taxa <- as.data.frame(tax_table(phy_rarefied)) %>%
  rownames_to_column("otu_id") %>%
  mutate(otu_id = factor(otu_id), 
