...
# R changed random number seed generation at R v.3.6.0, so this command is needed to rever to old seeds
...
In this step, we use the full order dataset (all OTUs classified to the level of Order) to characterize the core microbiome of each group of 

#### 4a. Characterize the core microbiome by phylogeny (L. africana or L. cyclotis); run the Rmd chunk below:

##### Characterize core microbiome by phylogeny:
```{r}
# Load order dataset
load("data/order_data.Rda")
