---
title: "Analysis 3: Core microbiome and differential OTU abundance analysis"
author: "Joe Gunn"
date: "2024-02-07"
...
## Specific Aim: Assessing the core microbiome and differential OTU abundance of African elephant sample groups (phylogeny, diet, and habitat) 
...
#### 3d. Summarize OTU proportions at four broadest taxonomic levels (kindgom, phylum, class, order) within each group of interest.
In this step, we calculate the proportion of the total microbiome represented by each present taxonomic group within kingdoms, phyla, classes, and orders in the sample set. We then break these proportions down by phylogeny, diet, and habitat for direct comparison of microbiota among groups. 

We ultimately present proportions of microbial OTUs at the phylum and order levels, giving relative proportions of each phylum and order across the full microbiome dataset, as well as the relative proportions of each order within each phylum. These summary statistics are the basis for Table S1 in the final manuscript. 

##### 3d.1. Summarize OTU proportions within groups in <i>L. africana</i>.

###### 3d.1.1. Summarize OTU abundance across taxanomic groups in savanna-dwelling, non-crop-riading <i>L. africana</i>; run the Rmd chunk below.

##### Summarize OTU abundance in savanna-dwelling, non-crop-riading <i>L. africana</i>
```{r}
# Load order dataset
load("data/order_data.Rda")
...
###### 3d.1.2. Summarize OTU abundance across taxanomic groups in savanna-dwelling, crop-riading <i>L. africana</i>; run the Rmd chunk below.

##### Summarize OTU abundance in savanna-dwelling, crop-riading <i>L. africana</i>
```{r}
# Load order dataset
load("data/order_data.Rda")
...
africana_raider <- order_data %>%
  filter(phylogeny == "africana" & habitat == "Savanna" & diet == "Raider")

# Calculate total microbial abundance across samples
total_abundance <- sum(africana_raider$abundance)
...
###### 3d.1.3. Summarize OTU abundance across taxanomic groups in forest-dwelling, non-crop-riading <i>L. africana</i>; run the Rmd chunk below.

##### Summarize OTU abundance in forest-dwelling, non-crop-riading <i>L. africana</i>
```{r}
# Load order dataset
load("data/order_data.Rda")
...
africana_forest <- order_data %>%
  filter(phylogeny == "africana" & habitat == "Forest" & diet == "Nonraider")

# Calculate total microbial abundance across samples
total_abundance <- sum(africana_forest$abundance)
...
##### Summarize OTU abundance in forest-dwelling, crop-riading <i>L. africana</i>
```{r}
# Load order dataset
load("data/order_data.Rda")
...
africana_forest_raider <- order_data %>%
  filter(phylogeny == "africana" & habitat == "Forest" & diet == "Raider")

# Calculate total microbial abundance across samples
total_abundance <- sum(africana_forest_raider$abundance)

# Get kingdom proportions
africana_forest_raider %>%
  group_by(kingdom) %>%
  summarize(kingdom_abundance = sum(abundance),
            kingdom_proportion = kingdom_abundance/total_abundance,
            kingdom_percent = kingdom_proportion*100)
...
full_abundance %>% 
  ...
  summarize(order_percent = order_abundance/phylum_abundance*100)
```
##### 3d.2. Summarize OTU proportions within groups in <i>L. cyclotis</i>.

###### 3d.2.1. Summarize OTU abundance across taxanomic groups in <i>L. cyclotis</i>; run the Rmd chunk below.
...
# Calculate total microbial abundance across samples
total_abundance <- sum(cyclotis$abundance)
...
### STEP 4. Summarize core microbiome by phylogeny, diet, and habitat
...
#### 4a. Characterize the core microbiome by phylogeny (<i>L. africana</i> or <i>L. cyclotis</i>).
...
All core microbiome proportions are the basis for Table S2 in the final manuscript.

##### 4a.1. Characterize the core microbiome of all <i>L. africana</i>; run the Rmd chunk below.
...
n_ind <- length(unique(savanna$ind_id))
...
savanna_core <- savanna %>%
  filter(order_proportion == 1.00000000)
```

##### 4a.2. Characterize the core microbiome of all <i>L. cyclotis</i>; run the Rmd chunk below.
...
n_ind <- length(unique(forest$ind_id))
...
##### 4a.3. Characterize the core microbiome of all <i>L. cyclotis</i> without individual M10; run the Rmd chunk below.
...
##### 4a.4. Characterize the core microbiome of only <i>L. africana</i> that were collected from forest habitat; run the Rmd chunk below.
...
savanna_forest <- order_data %>%
  filter(phylogeny == "africana" & habitat == "Forest" & diet == "Nonraider") 

# Get the total number of individual savanna forest elephants
n_ind <- length(unique(savanna_forest$ind_id))
...
savanna_forest_core <- savanna_forest %>%
  filter(order_proportion == 1.00000000)
```

#### 4b. Characterize the core microbiome by diet (crop-raider or non-crop-raider).
...
##### 4b.1. Characterize the core microbiome of all <i>L. africana</i> crop-raiders; run the Rmd chunk below.
...
n_ind <- length(unique(raider$ind_id))
...
##### 4b.2. Characterize the core microbiome of all <i>L. africana</i> non-crop-raiders; run the Rmd chunk below.
...
nonraider_core <- nonraider %>%
  filter(order_proportion == 1.00000000)
```

#### 4c. Characterize the core microbiome by habitat (savanna or forest).
...
##### 4c.1. Characterize the core microbiome of all <i>L. africana</i> savanna dwellers; run the Rmd chunk below.
...
##### 4b.2. Characterize the core microbiome of all <i>L. africana</i> forest dwellers; run the Rmd chunk below.
...
savanna <- savanna[,-c(3:9,11:12)]

savanna <- savanna %>%
  spread(otu_id, abundance)

savanna[, colSums(savanna == 0) == 0]

forest <- order_data %>%
  filter(phylogeny == "cyclotis")

forest <- forest[,-c(3:9,11:12)]

forest <- forest %>%
  spread(otu_id, abundance)

forest[, colSums(forest == 0) == 0]
```

```{r}

# Plot confidence and proportion of samples per taxonomic level
pdf("figures/order_composition.pdf", width = 12, height = 4) 

ggplot(orders, aes(x = ind_id, y = abundance, fill = order)) + 
  geom_bar(position = "stack", stat = "identity", show.legend = F,) +
  theme_set(theme_cowplot(12)) +
  labs(x = "Sample ID", y = "OTU abundance") +
  theme(axis.text = element_text(size = 15)) + 
  theme(axis.title = element_text(size = 15)) + 
  theme(axis.text.x = element_text(angle = 90)) +
  theme(axis.text.x = element_text(hjust = 0.95)) +
...
# Recreate long dataset so that elephant individuals are columns and orders are rows (for pcoa analysis)
orders <- orders %>%
  spread(key = ind_id, value = abundance)



# Convert first column to row names in preparation for data transposition (next step)
orders <- column_to_rownames(orders, "order")
  
# Create a data object with otu abundnace data and clean
orders_t <- as.data.frame(t(orders))

vegan <- decostand(orders_t, "total") #standardize abundance data by dividing each number by the total number of samples (individuals in the dataset)
...
pcs <- pcs %>% rownames_to_column("ind_id")

pcs <- merge(pcs, metadata, by = "ind_id")

ggplot(pcs, aes(x = Axis.1, y = Axis.2, label = ind_id, fill = species)) +
  geom_point() +
  geom_text()
...
<b><i>reads</i><sub>archaea</sub> = 4,818 </b><br>
