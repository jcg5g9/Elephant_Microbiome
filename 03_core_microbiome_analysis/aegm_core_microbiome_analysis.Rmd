...
# Exclude all taxonomic levels below order and drop NAs (to keep only OTUs that were classified to order level)
orders <- otu_abundance %>%
  select(otu_id, kingdom, phylum, class, order, ind_id, species) %>%
  drop_na()

# Sum read abundance by order
orders <- orders %>%
  group_by(order, ind_id, species, raider, habitat) %>%
